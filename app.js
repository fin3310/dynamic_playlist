...
function updateCardDOM(songId) {
    ...
    if (song) {
        ...
        if (card) { card.outerHTML = generateSongHtml(song); }
    }
}
...
function renderFilters() {
    ...
    const shortMap = { '日文': '日', '中文': '中', '英文': '英', '韓文': '韓', '全部': '全' };
...
function resetAndRender() {
    ...
    if (currentSubTag !== '全部') filterStatus += ` > ${currentSubTag}`;
    if (searchQuery) filterStatus += ` (搜尋: "${searchQuery}")`;
    ...
    if (countDisplay) countDisplay.innerText = `共 ${currentFilteredSongs.length} 首`;
